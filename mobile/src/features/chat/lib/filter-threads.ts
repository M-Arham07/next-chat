...
const filterThreads = (
    ...
    result = result.filter((thread) => {
        // Check if query matches messages in this thread
        const matchesMsgs = messages?.[thread.threadId]?.some((msg) => msg.content.toLowerCase().includes(query));

        // Check if query matches participant names (excluding current user)
        let matchesName = false;

        if (
            thread.participants.some(
                (p) => p.username.toLowerCase().includes(query) && p.username !== profile?.username?.toLowerCase()
            )
        ) {
            matchesName = true;
