...
export async function POST(
    ...
    try {
        ...
        const body = rekeySchema.parse(await request.json());
        const supabase = await createClient();

        const { data, error } = await supabase.rpc("rotate_thread_e2ee_key", {
        ...
        return NextResponse.json(
            { error: error instanceof Error ? error.message : "Failed to rotate thread key" },
            { status: 500 },
        );
    }
}
