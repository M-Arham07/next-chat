...
type GlobalLoaderProps = {
  loader?: LoaderType;
  text?: string; // used by LoaderFour/LoaderFive if needed
  fullscreen?: boolean;
  className?: string;
};
...
export default function GlobalLoader({
  ...
  if (!loading) return null; // early exit ! 


  const loaders: Record<LoaderType, React.ReactNode> = {
    LoaderOne: <LoaderOne />,
    LoaderTwo: <LoaderTwo />,
    LoaderThree: <LoaderThree />,
    LoaderFour: <LoaderFour text={text} />,
    LoaderFive: <LoaderFive text={text} />,
  };

  const content = loaders[loader] ?? <LoaderOne />;

  // Fullscreen overlay 
  if (fullscreen) {
    return (
      <div
        className={`fixed inset-0 z-9999 grid place-items-center bg-black/40 backdrop-blur-sm ${className}`}
      >
        {content}
      </div>
    );
...
const LoaderFour = ({ text = "Loading..." }: { text?: string }) => {
  return (
